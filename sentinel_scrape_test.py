...
api = SentinelAPI('gijstimmerij', 'Sjiggie007')
...
coordinates = [(76.89941158791933, 17.223384891246383), (77.00050005239353, 17.223384891246383), (77.00050005239353,
                                                                                                  17.276782301151), (76.89941158791933, 17.276782301151), (76.89941158791933, 17.223384891246383)]
...
def product_list(coordinates, start_date, end_date):
    ...
    products = api.query(footprint, date=("%sT00:00:00.000Z" % (
        start_date), "%sT23:59:59.999Z" % (end_date)), producttype='S2MSI1C')
    return products


def download(key, start_date, end_date):
    path = r'C:\Users\gijst\Documents\Master Data Science\Agurotech\Crop_yield_prediction\Data\%s_%s' % (
        start_date, end_date)
...
while (count <= total_images-monthly_gap+1):
    if count == 1:
        ...
        if not os.path.exists(r'C:\Users\gijst\Documents\Master Data Science\Agurotech\Crop_yield_prediction\Data\%s_%s\%s.zip' % (start_date, end_date, item_title)):
        ...
        if len(a_dict) == 1:
            if not os.path.exists(r'C:\Users\gijst\Documents\Master Data Science\Agurotech\Crop_yield_prediction\Data\%s_%s\%s.zip' % (start_date, end_date, item_title1)):
